...
object DatabaseModule {
    ...
    private val migration13To14 = object : Migration(13, 14) {
        override fun migrate(database: androidx.sqlite.db.SupportSQLiteDatabase) {
            ...
            database.execSQL("CREATE UNIQUE INDEX IF NOT EXISTS `index_local_audio_tracks_uri` ON `local_audio_tracks` (`uri`)")
        }
    }
    ...
    @Provides
    @Singleton
    fun provideLocalAudioTrackDao(database: AppDatabase): com.lastwave.app.data.local.db.LocalAudioTrackDao =
        database.localAudioTrackDao()
}
