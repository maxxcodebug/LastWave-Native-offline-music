...
data class MiscSettings(
    ...
    /** Enables the local music service so the app can play the selected library without network access. */
    val offlineServiceEnabled: Boolean = false,
    /** Persisted SAF tree URI containing the user-selected local music folder. */
    val offlineMusicTreeUri: String? = null,
)
...
class SettingsPreferences @Inject constructor(
    ...
    private object Keys {
        ...
        val OFFLINE_SERVICE_ENABLED = booleanPreferencesKey("lw_offline_service_enabled")
        val OFFLINE_MUSIC_TREE_URI = stringPreferencesKey("lw_offline_music_tree_uri")
    }
        ...
        .map { p ->
            MiscSettings(
                ...
                hiddenHomeSections = p.readSafely(Keys.HIDDEN_HOME_SECTIONS)
                    ...
                    ?: emptySet(),
                offlineServiceEnabled = p.readSafely(Keys.OFFLINE_SERVICE_ENABLED) ?: false,
                offlineMusicTreeUri = p.readSafely(Keys.OFFLINE_MUSIC_TREE_URI),
            )
        }
    ...
    suspend fun setOfflineServiceEnabled(enabled: Boolean) {
        dataStore.edit { it[Keys.OFFLINE_SERVICE_ENABLED] = enabled }
    }

    suspend fun setOfflineMusicTreeUri(uri: String?) {
        dataStore.edit {
            if (uri.isNullOrBlank()) it.remove(Keys.OFFLINE_MUSIC_TREE_URI)
            else it[Keys.OFFLINE_MUSIC_TREE_URI] = uri
        }
    }
